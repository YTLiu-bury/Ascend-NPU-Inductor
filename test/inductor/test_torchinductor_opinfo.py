...
def print_seen():
    ...
    for device_type in ("cpu", NPU_TYPE):
...
# requires_npu decorator
requires_npu = unittest.skipIf(not HAS_NPU, "requires npu")
...
# NPUTestBase - similar to CPUTestBase but for NPU
# Inherits from DeviceTypeTestBase to support test instantiation
from torch.testing._internal.common_device_type import DeviceTypeTestBase

class NPUTestBase(DeviceTypeTestBase):
    device_type = NPU_TYPE  # NPU device type

    # No critical error should stop NPU test suite
...
if not SM80OrLater:
    inductor_skips["npu"]["bfloat16"] = {b8, f16, f32, f64, i32, i64}

if TEST_WITH_ROCM:
    # Tensors are not alike
    inductor_skips["npu"]["logcumsumexp"] = {f32}
    inductor_skips["npu"]["special.modified_bessel_i1"] = {f64}
...
inductor_expected_failures_single_sample["npu"] = {
...
inductor_expected_failures_single_sample["npu"].update(intentionally_not_handled)
inductor_expected_failures_single_sample["xpu"].update(intentionally_not_handled)


inductor_gradient_expected_failures_single_sample = defaultdict(dict)

inductor_gradient_expected_failures_single_sample["npu"] = {
    # float16 scalar gradient underflows to inf in CPU/NPU eager reference.
    # Compiled kernel avoids underflow and returns correct gradient (1.0).
    # Repro:
    #   x = torch.tensor(0.0703125, dtype=torch.float16, device='npu:0', requires_grad=True)
    #   out = torch.linalg.vector_norm(x, ord=6)
    #   out.backward()
    #   # CPU eager grad:  inf  |  NPU eager grad:  inf  |  NPU compiled grad:  1.0
...
inductor_should_fail_with_exception["npu"] = {}
...
# NPU gradient expected failures for the dynamically-created TestInductorOpInfoNPU class
npu_gradient_expected_failures = get_skips_and_xfails(
    inductor_gradient_expected_failures_single_sample, xfails=True
)
...
inductor_override_kwargs["npu"] = {
    ...
    # hinge_embedding_loss: NPU compiled (fp16) matches NPU eager (fp16) exactly.
    # The test computes reference in fp32 then downcasts to fp16, introducing
    # ~0.00025 abs / ~0.02% rel fp32->fp16 rounding that exceeds default rtol.
    ("nn.functional.hinge_embedding_loss", f16): {"atol": 0.001, "rtol": 0.005},
    ...
    # NPU eager gelu_backward is less accurate than NPU compiled backward vs CPU.
    # Compiled backward matches CPU (max_abs~1e-08) but eager diverges (max_abs~1e-04).
    # The test compares compiled vs eager, so we relax gradient tolerance.
    ("nn.functional.gelu", f32): {"grad_atol": 0.001, "grad_rtol": 0.05},
}
...
# Manually instantiate tests for NPU only
# instantiate_device_type_tests doesn't support "npu" as a device type
# Create NPU-specific test class that inherits from NPUTestBase and TestInductorOpInfo
TestInductorOpInfoNPU = type(
    'TestInductorOpInfoNPU',
    (NPUTestBase, TestInductorOpInfo,),
    {'device_type': NPU_TYPE, 'device': NPU_TYPE}
)
globals()['TestInductorOpInfoNPU'] = TestInductorOpInfoNPU

# Apply expected failures for NPU test class (skipOps was applied to TestInductorOpInfo
# before this class existed, so DecorateInfo test_case_name doesn't match)
skipOps("TestInductorOpInfoNPU", "test_comprehensive", npu_gradient_expected_failures)

# Instantiate the test methods
test = getattr(TestInductorOpInfoNPU, 'test_comprehensive', None)
if test:
    TestInductorOpInfoNPU.instantiate_test('test_comprehensive', test, generic_cls=TestInductorOpInfoNPU)

# Remove the generic test class so tests are not discovered twice
del TestInductorOpInfo
