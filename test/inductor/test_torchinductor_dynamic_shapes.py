...
# IS_SM89 equivalent for NPU - always False for compatibility
IS_SM89 = False
...
test_failures = {
    ...
    "test_randint_distribution_dynamic_shapes": TestFailure(("npu", "xpu")),
}
...
if os.getenv("BUILD_ENVIRONMENT", "").endswith("-debug"):
    ...
    test_failures["test_resize_as_dynamic_shapes"] = TestFailure(("cpu", "npu"))
    test_failures["test_resize_dynamic_shapes"] = TestFailure(("cpu", "npu"))
...
# CPU tests disabled for NPU testing
# if HAS_CPU:
...
if HAS_NPU and not TEST_WITH_ASAN:

    class DynamicShapesNPUTests(TestCase):
        common = check_model_gpu
        device = NPU_TYPE

    copy_tests(
        DynamicShapesCommonTemplate, DynamicShapesNPUTests, NPU_TYPE, test_failures
    )
...
class TestInductorDynamic(TestCase):
    ...
    def test_cat_unbacked_duplicate_size(self, device):
        ...
        cf = torch.compile(fullgraph=True)(f)
        arg = torch.tensor([4, 6], device=NPU_TYPE)
        self.assertEqual(f(arg), cf(arg))
    ...
    # @onlyCPU - skipped for NPU testing
    ...
    def test_mark_unbacked_slice(self):
        ...
        x = torch.empty_strided((1, 4), (5, 1), device=NPU_TYPE)
    ...
    def test_sort_dynamic_shape_with_check(self, device):
        if torch.device(device).type != NPU_TYPE:
...
if __name__ == "__main__":
    ...
    if (HAS_CPU or HAS_NPU) and not TEST_WITH_ASAN:
