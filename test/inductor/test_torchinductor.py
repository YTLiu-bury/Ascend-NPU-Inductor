...
import torch
# Note: transfer_to_npu is not imported here because it replaces torch.cuda.*
# with torch.npu.*, which causes get_gpu_type() to fail (it sees both cuda
# and npu as available). The inductor tests work correctly without this import.
# try:
#     from torch_npu.contrib import transfer_to_npu
# except ImportError:
#     pass  # Fallback for non-NPU environments
...
# NPU-specific definitions for migrated tests
# transfer_to_npu handles runtime conversions for torch.cuda.* -> torch.npu.*
NPU_TYPE = "npu"
HAS_NPU = torch.npu.is_available() if hasattr(torch, 'npu') else False
HAS_MULTINPU = HAS_NPU and torch.npu.device_count() > 1 if hasattr(torch, 'npu') else False
TEST_NPU = HAS_NPU

# SM80OrLater equivalent for NPU - always True for compatibility
SM80OrLater = True

# TEST_CUDNN equivalent for NPU - always True for compatibility
TEST_CUDNN = True

# PLATFORM_SUPPORTS_FLASH_ATTENTION equivalent for NPU
PLATFORM_SUPPORTS_FLASH_ATTENTION = True

# PLATFORM_SUPPORTS_MEM_EFF_ATTENTION equivalent for NPU
PLATFORM_SUPPORTS_MEM_EFF_ATTENTION = True

requires_npu = functools.partial(unittest.skipIf, not HAS_NPU, "requires npu")
requires_multinpu = functools.partial(unittest.skipIf, not HAS_MULTINPU, "requires multiple npu devices")

# skipNPUIf - decorator that conditionally skips tests
def skipNPUIf(condition, reason):
    """Decorator to skip tests if condition is True."""
...
requires_multigpu = functools.partial(
    unittest.skipIf, not HAS_MULTIGPU, f"requires multiple {NPU_TYPE} devices"
)
...
def check_model_gpu(
    ...
    *,
    ...
    kwargs = kwargs or {}
    if hasattr(model, "to"):
        model = model.to(device=NPU_TYPE)

    if copy_to_gpu:
        example_inputs = tuple(
            clone_preserve_strides_offset(x, device=NPU_TYPE) for x in example_inputs
        )
    ...
    if check_lowp:
        ...
        def downcast_fn(x):
            ...
            return torch.empty_strided(
                x.size(), x.stride(), device=NPU_TYPE, dtype=torch.half
            ).copy_(x)
...
class CommonTemplate:
    ...
    @skipNPUIf(not SM80OrLater, "Requires sm80")
    @skip_if_halide  # aoti
    ...
    @skipNPUIf(not SM80OrLater, "uses bfloat16 which requires SM >= 80")
    ...
    def test_multilayer_sum_low_prec(self):
        # fp16 nyi for cpu
        if self.device == "cpu":
            raise unittest.SkipTest(f"requires {NPU_TYPE}")
    ...
    @skipNPUIf(not SM80OrLater, "Requires sm80")
    @skipNPUIf(TEST_WITH_ROCM, "Computation not done in float on ROCm")
    ...
    @skipNPUIf(TEST_WITH_ROCM, "associative_scan is not supported on ROCm")
    ...
    def test_embedding_bag_byte_unpack(self):
        if self.device != "cpu":
            raise unittest.SkipTest(f"No {NPU_TYPE} implementation (it returns empty)")
    ...
    @skipNPUIf(True, "No _weight_int8pack_mm implementation on CUDA")
    ...
    @xfail_if_triton_cpu
    @skipNPUIf(True, "No _dyn_quant_pack_4bit_weight implementation on CUDA")
    @skipIfRocm
    ...
    @xfail_if_triton_cpu
    @skipNPUIf(True, "No _dyn_quant_matmul_4bit implementation on CUDA")
    @skipIfRocm
    ...
    def test_linspace4(self):
        def fn(x):
            return torch.linspace(0, 2, 0, device=f"{NPU_TYPE}:1")
    ...
    @skipNPUIf(True, "Not implemented for CUDA")
    ...
    def test_large_pointwise(self):
        ...
        del t
        if torch.device(self.device).type == NPU_TYPE:
            getattr(torch, NPU_TYPE).empty_cache()
    ...
    @skipNPUIf(True, "cuda failed for float64 linear")
    ...
    def test_conv_inference_heuristics(self):
        if self.device != NPU_TYPE:
            raise unittest.SkipTest(f"{NPU_TYPE} only test")
        ...
        def fn(a):
            if a.device.type == "cpu":
                return aten._to_copy(
                    a, device=torch.device(NPU_TYPE), dtype=6, layout=0
                )
            else:
    ...
    def test_multi_gpu_device(self):
        ...
        x = torch.rand([4], device=NPU_TYPE)
    ...
    def test_multi_gpu_recompile_on_index(self):
        ...
        x0 = torch.randn(1024, 1024, device=f"{NPU_TYPE}:0")
        y0 = torch.randn(1024, 1024, device=f"{NPU_TYPE}:0")

        gemm_opt(x0, y0)

        x1 = torch.randn(1024, 1024, device=f"{NPU_TYPE}:1")
        y1 = torch.randn(1024, 1024, device=f"{NPU_TYPE}:1")
    ...
    def test_batch_norm_2d_2(self):
        ...
        inp = torch.randn((4, 64, 192, 256), dtype=torch.float32, device=NPU_TYPE)
        mod = Repro().to(device=NPU_TYPE)
        o1 = mod(inp)
    ...
    def test_masked_fill_promotion(self):
        ...
        for inp in (
            torch.randn(
                [16, 16],
                dtype=torch.float16 if self.device == NPU_TYPE else torch.float32,
                device=self.device,
            ),
    ...
    @skipNPUIf(not TEST_CUDNN, "CUDNN not available")
    @skipIfXpu
    @skipIfRocm
    ...
    def test_philox_rand(self):
        if self.device == "cpu":
            raise unittest.SkipTest(
                f"functionalization of rng ops supported only on {NPU_TYPE}"
            )
    ...
    def test_randint(self):
        ...
        def fn(x):
            return (
                ...
                torch.randint_like(x, 2**50, dtype=torch.int64),
            )
    ...
    def test_like_rands3(self):
        ...
        a0 = test_like_rands_on_different_device("cpu", NPU_TYPE)
        a1 = test_like_rands_on_different_device(NPU_TYPE, "cpu")
        self.assertTrue(a0.device.type == NPU_TYPE)
    ...
    def test_unspec_inputs(self, dtype):
        ...
        opt = torch.compile(fn, backend="inductor")
        inputs = (
            rand_strided((2, 3), (3, 1), dtype=torch.float32, device=NPU_TYPE),
    ...
    def test_shape_padding(self):
        ...
        def gen(*shape, dtype=torch.float32):
            return torch.randn(*shape, device=NPU_TYPE, dtype=dtype) / k + 1.0
    ...
    def test_stride_preservation_with_stride_modifying_fx_pass(self):
        ...
        with config.patch(
            ...
            x = torch.rand(4, 4, device=NPU_TYPE)
            y = f(x)
    ...
    def test_sdpa(self, use_block_ptr: bool, prefer_nd_tiling: bool):
        ...
        DEVICE = torch.device(f"{NPU_TYPE}:0")
        DTYPE = torch.float16
        B = 3
        H = 8
        Q = 99
    ...
    def test_sdpa_unaligned_mask(self):
        ...
        query = torch.rand(8, 8, 16, 16, device=NPU_TYPE)
        key = torch.rand(8, 8, 15, 16, device=NPU_TYPE)
        value = torch.rand(8, 8, 15, 16, device=NPU_TYPE)
        bias = torch.rand(1, 1, 16, 15, device=NPU_TYPE)
        self.common(
            foo,
    ...
    def test_sdpa_unaligned_mask_freezing(self):
        class Mod(torch.nn.Module):
            def __init__(self) -> None:
                super().__init__()
                self.arg3_1 = torch.rand(1, 1, 16, 15, device=NPU_TYPE)
    ...
    def test_custom_op_fixed_layout_channels_last(self):
        ...
        model = Block()
        model = model.to(NPU_TYPE).to(memory_format=torch.channels_last)
        input_t = torch.randn([1, 320, 128, 128], dtype=torch.float32, device=NPU_TYPE)
    ...
    def test_pointwise(self, name, op):
        dtype = torch.float32
        check_lowp = True
        if self.device == NPU_TYPE and name in {
            "airy_ai",
            "bessel_i0",
            "bessel_i1",
            "bessel_j0",
    ...
    def test_mix_device_index(self):
        ...
        """
        image_latent = (
            torch.randn((24, 16, 32, 32), device=NPU_TYPE)
            .to(memory_format=torch.channels_last)
            .view(2, 12, 16, 32, 32)
        )
    ...
    def test_prepare_softmax_with_fast_math(self):
        ...
        if DO_PERF_TEST:
            ...
            M = 128
            N = 128
        x = torch.randn(M, N, dtype=torch.bfloat16, device=NPU_TYPE)
...
if HAS_NPU:
    ...
    class SweepInputsNPUTest(SweepInputs2, TestCase):
        gen = InputGen(10, NPU_TYPE)

    SweepInputsNPUTest.populate()

    class NPUTests(TestCase):
        common = check_model_gpu
        device = NPU_TYPE

    copy_tests(CommonTemplate, NPUTests, NPU_TYPE)
    ...
    class TritonCodeGenTests(TestCase):
        ...
        def test_divisible_by_16_covers_numel_args(self):
            ...
            kernels = self.get_kernels(fn, [torch.randn([256, 256], device=NPU_TYPE)])
            expected_divisible = {
        ...
        def test_codegen_config_option_dont_assume_alignment(self):
            ...
            for offset in (0, 1, 2, 3, 4):
                base = torch.randn(64 * 64 + 64, dtype=torch.float32, device=NPU_TYPE)
            ...
            torch._dynamo.reset()
            inp = torch.randn((64, 64), device=NPU_TYPE)
        ...
        def test_optimize_indexing_dtype(self):
            ...
            fn_opt = torch.compile(fn, backend="inductor")
            inps = [torch.randn(2, 4, 16, 16, device=NPU_TYPE)]
            code = run_and_get_triton_code(fn_opt, *inps)
        ...
        def test_optimize_indexing_dtype_with_constraint(self):
            def fn1(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
                x = torch.arange(0, b.shape[0], device=NPU_TYPE)
                y = ((x + x) / 3).int()
            ...
            a = torch.rand([100, 100], device=NPU_TYPE)
            b1 = torch.rand([102], device=NPU_TYPE)
            b2 = torch.rand([100], device=NPU_TYPE)
        ...
        def test_inductor_detach_view(self, backend):
            ...
            inp = torch.ones(2, 2, requires_grad=True, device=NPU_TYPE)
        ...
        def test_sdpa_inference_mode_aot_compile(self):
            ...
            q = torch.rand([10, 4, 128, 64], device=NPU_TYPE, dtype=torch.bfloat16)
            k = torch.rand([10, 4, 128, 64], device=NPU_TYPE, dtype=torch.bfloat16)
            v = torch.rand([10, 4, 128, 64], device=NPU_TYPE, dtype=torch.bfloat16)
            attn_mask = (
                torch.rand([10, 4, 128, 128], device=NPU_TYPE, dtype=torch.bfloat16)
                < 0.9
            )
        ...
        def test_optimize_indexing_assert(self):
            ...
            for dynamic in (False, True):
                ...
                x = torch.randn(8, device=NPU_TYPE)
            ...
            for dynamic in (False, True):
                fn_opt = torch.compile(fn, dynamic=dynamic)
                a = torch.randn(1, 32, 32, 4, device=NPU_TYPE)
                z = torch.zeros((), dtype=torch.int64, device=NPU_TYPE)
                b = torch.randn(33, 1, device=NPU_TYPE)
                idx0 = torch.randint(32, (33,), device=NPU_TYPE).view(33, 1, 1)
                idx1 = torch.randint(32, (33,), device=NPU_TYPE).view(33, 1)
        ...
        def test_not_materialize_pointwise_reduction(self):
            ...
            fn_opt = torch.compile(fn, backend="inductor")
            inps = [
                torch.randn(N, 1, K, device=NPU_TYPE),
                torch.randn(1, N, K, device=NPU_TYPE),
            ]
            code = run_and_get_triton_code(fn_opt, *inps)
            self.assertEqual(
        ...
        def test_cant_optimize_compute(self):
            def ones():
                return torch.ones([4], device=NPU_TYPE)

            def suffix(inp):
                return (inp.to(torch.int64) + 1).to(torch.float64)

            ten = torch.rand([4], device=NPU_TYPE)
        ...
        def test_ctr_not_moved_to_cuda_when_used_in_index_put(self):
            ...
            x_tmp = torch.randn(512, 19, device=NPU_TYPE)
            x = x_tmp.permute(1, 0).view(-1, 128, 4)[:, :, 1:]

            mask_tmp = torch.ones(128, 3, dtype=torch.int32, device=NPU_TYPE)
        ...
        def test_evict_last_non_coalesced_loads(self):
            ...
            N = 512
            inps = (
                torch.randn(N, N, N, device=NPU_TYPE).permute(2, 1, 0),
                torch.randn(N, N, N, device=NPU_TYPE).permute(1, 2, 0),
            )
    ...
    tmp1 = tl.load(in_ptr1 + (x3 + (262144*r2)), rmask, other=0.0)
        ...
        def test_skip_l1_cache(self):
            ...
            N = 512
            inps = (torch.randn(N, device=NPU_TYPE), torch.randn(N, device=NPU_TYPE))
            code = run_and_get_triton_code(f, *inps)
            lines = [line for line in code.split("\n") if "tl.load" in line]
            self.assertExpectedInline(
                "\n".join(lines),
    ...
    tmp1 = tl.load(tl.make_block_ptr(in_ptr1, shape=[262144, 512], strides=[1, 262144], block_shape=[XBLOCK, R0_BLOCK], order=[0, 1], offsets=[xoffset, roffset]), boundary_check=[1], padding_option='zero')
        ...
        def test_has_constant_mask(self, block_multiple, ynumel_exceed_ygrid_size):
            ...
            a = torch.zeros(shape, device=NPU_TYPE, dtype=torch.bool)
            b = torch.zeros((shape[0], 1), device=NPU_TYPE, dtype=torch.bool)
        ...
        def test_kernel_names_descriptive(self):
            ...
            func_and_kernel_aten = [
                (fn1, "triton_poi_fused_cos_sin", (torch.randn(8, device=NPU_TYPE),)),
                (
                    fn2,
                    "triton_poi_fused__softmax",
                    (torch.randn(4, 4, device=NPU_TYPE),),
                ),
                (
                    fn3,
        ...
        def test_bandwidth_profiler(self):
            ...
            inp = torch.randn(4, 4, device=NPU_TYPE)
        ...
        def test_inductor_sequence_nr(self):
            ...
            x = torch.rand(100, 16, 32, 32, requires_grad=True, device=NPU_TYPE)
            target = torch.rand(1, device=NPU_TYPE)
            args = [x, target]
            model = Model().to(device=NPU_TYPE)
        ...
        @expectedFailureXPU
        @unittest.skipIf(
            torch.npu.is_available() and (9, 0) < (9, 0),
        ...
        def test_red_followed_by_transposed_pointwise(self):
            ...
            in1 = torch.randn((bs, dim), dtype=torch.bfloat16, device=NPU_TYPE)
            in2 = torch.randn((bs, dim), dtype=torch.bfloat16, device=NPU_TYPE)
            a = (
                torch.randn((dim, dim), dtype=torch.bfloat16, device=NPU_TYPE)
                .t()
                .to(torch.float8_e4m3fn)
            )
            b = torch.randn((dim, bs), dtype=torch.bfloat16, device=NPU_TYPE).to(
                torch.float8_e4m3fn
            )
            # Scales
            scale_a = torch.tensor(1.0, device=NPU_TYPE)
            scale_b = torch.tensor(1.0, device=NPU_TYPE)
        ...
        def test_layer_norm_inplaces_after_matmul(self):
            ...
            layer_norm = torch.nn.LayerNorm(hidden_size, device=NPU_TYPE)
            ...
            inps = [
                torch.randn(batch_size, seq_length, hidden_size, device=NPU_TYPE),
                torch.randn(hidden_size, hidden_size, device=NPU_TYPE),
            ]
            fn_opt = torch.compile(fn)
            code = run_and_get_triton_code(fn_opt, *inps)
        ...
        def test_donated_buffer_inplace_gpt(self):
            ...
            B, T = 1, 1024
            ctx = torch.amp.autocast(device_type=NPU_TYPE, dtype=torch.bfloat16)
            ...
            x = torch.randint(0, 50257, (B, T), dtype=torch.int64, device=NPU_TYPE)
            y = torch.randint(0, 50257, (B, T), dtype=torch.int64, device=NPU_TYPE)
        ...
        def test_triton_attrs_dict_constexpr_signature(self):
            ...
            fn_c = torch.compile(fn)
            x = torch.rand(16, device=NPU_TYPE)
        ...
        def test_graph_partition(self):
            def f(x, y):
                ...
                y_cpu = y1.cpu() + 1
                z = x @ y
                return x1 + y1 + z + y_cpu.to(NPU_TYPE)
        ...
        def test_graph_partition_buffer_reuse(self):
            def f(x, y):
                ...
                y_cpu = y1.cpu() + 1
                z = x1 + y1 + x @ y
                u = (y_cpu.to(NPU_TYPE) + 2) @ y + 3
                u_cpu = u.cpu() + 2
                return z + u_cpu.to(NPU_TYPE)

            x, y = [torch.ones(2, 2, device=NPU_TYPE) for _ in range(2)]
            x_cloned, y_cloned = [tmp.clone() for tmp in [x, y]]
            eager_out = f(x, y)
    ...
    class NanCheckerTest(TestCase):
        ...
        def test_nan_checker_pass(self):
            ...
            x = torch.randn(2, 1024, device=NPU_TYPE)
            ref = f(x)
